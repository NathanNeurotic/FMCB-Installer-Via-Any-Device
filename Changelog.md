...
- reworked what the installer copies, and where:
  * **`INSTALL/APPS`** — on a memory card install its **contents** go to the card's `APPS` folder (unchanged); on an HDD install the **whole folder** is now installed to the standard **`__common`** partition as `__common:/APPS`
  ...
  * the dedicated `PP.FHDB.APPS` HDD partition is no longer created (so `SYSTEM/AICON.SYS`, `SYSTEM/AICON.ICN` and `SYSTEM/ASYSTEM.CNF`, which only ever supplied that partition's HDD-OSD icon/attributes, are now unused)
